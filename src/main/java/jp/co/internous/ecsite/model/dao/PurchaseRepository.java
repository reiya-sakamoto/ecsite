...
public interface PurchaseRepository extends JpaRepository<Purchase,Long>{
	@Query(value="SELECT*FROM parchase p "+
			"WHERE created_at=("+
			"SELECT MAX(created_at) FROM parchase p WHERE p.user_id=:userId)",
			nativeQuery=true)
	List<Purchase> findHistory(@Param("userId") long userId);
	
	@Query(value="INSERT INTO parchase (user_id,goods_id,goods_name,item_count,total,created_at)"+
